...
**Scope** — `scripts/**`
**Last updated** — 2026-08-06, first partial-fund run (🔴→✅ **`GLD-ACQ-21`** the recycle's refusal scaffold swallowed its own output whenever ANY series funded — fixed. Previous: session 96 — **`GLD-ACQ-18`** whole-household mandate; **`GLD-ACQ-19`** recycle keyed on a never-populated column)
...
Eleven edits have been made. **Everything else in this register is documentation
only.**
...
|---|---|---|---|---|
| 1 | `GLD-REG-11` | 🔴 Fix the inverted registry anomaly heuristic | S | **Confirmed defect.** Flags every row today and would stay silent on a genuine stale-mirror import — the exact inverse of its purpose |
...
|---|---|---|---|---|
| `GLD-REG-11` | Registry anomaly heuristic is inverted — flags the canonical checkout, silent on the stale one. `_is_expected_path()` is written but never called | S | 🔵 Open | [registry §6.1](./managers/factories/registry/DESIGN.md) |
...
| `GLD-REG-03` | `print_tree_view` called by `base_manager.py` but does not exist | S | 🔵 Open | [registry §9](./managers/factories/registry/DESIGN.md) |
...
| `GLD-FAC-04` | Remove or implement `print_tree_view` *(dup of `GLD-REG-03`)* | S | 🔵 Open |
...
| `GLD-REG-03` | 🔴 Implement or remove `print_tree_view` | S | 🔵 Open |
...
| `GLD-REG-11` | 🔴 Fix inverted anomaly heuristic; wire in `_is_expected_path()` | S | 🔵 Open |
...
| `GLD-ACQS-06` | Track the component matrix across runs — rendered and discarded | M | 🔵 Open |
