...
| `GLD-DEL-10` | **Wire the `upgraded` disposition** — `run_jit_quality_upgrades` / `run_active_watcher_upgrades` record nothing, so `detect_churn` can never fire and `space_ledger` counts reclaim without spend | ✅ **Done** — both services now record an intent worklist at trigger and reconcile it at the START of the next pass, re-pointing landed upgrades so they never become orphans. Completed by `GLD-DEL-12` (Radarr) and `GLD-DEL-13` (archive emission) | M | `GLD-DEL-06` |
| `GLD-DEL-12` | **Reconcile Radarr's upgrades too** — `run_active_watcher_upgrades` fires a search and keeps no trace, so movie rows still go stale on every upgrade | ✅ **Done** — two adapters over the media-agnostic pure half (P-E). Movies key as `movie:<id>` in their own namespace, so a Sonarr and a Radarr worklist can never collide on a shared numeric id | S | `GLD-DEL-10` |
| `GLD-DEL-13` | **Emit `upgraded` / `upgrade-abandoned` rows into the archive** on reconciliation, so intent-vs-outcome is auditable and `detect_churn` has both directions | ✅ **Done** — emitted at RECONCILIATION rather than at trigger, so `replaced_by` carries a MEASURED size and `state: "imported"` instead of a projection. Measured on the closed loop: spend 0 → 12 GB, churn flips 0 → 1 | S | `GLD-DEL-10` |
| `GLD-DEL-11` | **Reconcile intent against outcome** — a `stepped-down` row's `replaced_by` carries `state: "queued"`, meaning the *arr ACCEPTED the grab, not that it imported. Nothing ever goes back to confirm | *(**P-D**)* Without it the archive asserts replacements that may never have landed, and `space_ledger` counts their bytes. Per-FILE reconciliation only — the parquet is a working set, so absence there cannot distinguish deleted from never-synced *(**P-C**)*; it must ask the *arr about the specific id. **Now tractable**: `GLD-DEL-13` proved the shape on the upgrade side, and `reconcile_upgrades` is media- and reason-agnostic, so the step-down trigger only needs to record an intent the same way | M | `GLD-DEL-05`, `GLD-DEL-13` |
