...
Config keys read: `tautulli` (instance configs), `free_space_limit`, `grace_window_ramp`, top-level `universe_membership` ("tags" default | "derived" | "hybrid" — see `quality/universe_membership.py`), and `radarr_instances` indirectly via instance resolution.
Global_cache keys read: `radarr.movies.<instance>.full`, `radarr.tags.<instance>`, `radarr.quality.<instance>`; in derived/hybrid membership modes also `plex/playlists/universe_source` + `plex/playlists/kometa_franchises` (via `gather_derived_maps`), and it WRITES the counts blob `radarr/<instance>/universe_membership`. Parquet written: `radarr/<instance>/movie_files.parquet`.
...
- `classification.keep_policy.build_keep_policy_map` (via `_build_keep_policy_map`) — produces the keep-policy + universe-name maps. `refresh` routes through the mode-aware `_resolve_membership_maps`: in the default `universe_membership: "tags"` it delegates to this VERBATIM (byte-identical maps and logs); in "derived"/"hybrid" it calls `quality/universe_membership.build_membership_maps`, which fills untagged movies from TMDB collections / learned franchise maps / mdblist universe lists with BARE-universe policy (`keep_policy='universe'` — deletable as last resort; the `keep-universe` never-delete pin remains explicit-tag-only), logs ONE per-instance line with membership counts by source, and stashes the counts at `radarr/<instance>/universe_membership` for the universe audit.
