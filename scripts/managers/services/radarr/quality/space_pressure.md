...
- **Delete ceiling (`space_pressure_score_ceiling`, default 17)**: an unwatched movie is only delete-eligible if its score is `< 17`. Worked example: an unwatched movie scoring 14 (below the 17 ceiling), with `date_added` older than 30 days, becomes a tier-1 delete candidate; one scoring 25 is skipped. A movie with NO `watchability_score` yet is DEFERRED, not scored with a fallback — it re-qualifies once `refresh_scores` reaches it, and is counted as `unscored (deferred)` in the coordinator's pool table.
