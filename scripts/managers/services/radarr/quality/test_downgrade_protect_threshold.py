...
the delete ceiling (space_pressure_score_ceiling, default 17 — re-anchored from 20 when Group D
v2 translated the score axis) so any deletable title is shrunk to 720p first. A stub manager
...
def test_flag_widens_to_delete_ceiling():
    # Flag on → match the delete band (default ceiling 17).
    assert _mgr({"space_pressure_downgrade_before_delete": True})._downgrade_protect_threshold() == 17
...
def test_flag_with_malformed_ceiling_falls_back_to_the_default():
    cfg = {"space_pressure_downgrade_before_delete": True, "space_pressure_score_ceiling": "oops"}
    assert _mgr(cfg)._downgrade_protect_threshold() == 17
