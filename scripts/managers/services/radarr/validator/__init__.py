...
from scripts.managers.services.radarr.validator.auth import RadarrValidatorAuthManager
from scripts.managers.services.radarr.validator.cache import RadarrValidatorCacheManager
from scripts.managers.services.radarr.validator.health import RadarrValidatorHealthManager
from scripts.managers.services.radarr.validator.keys import RadarrValidatorKeysManager
from scripts.support.utilities.decorators.timing import timeit
from scripts.support.utilities.logger.logger import LoggerManager
from scripts.support.utilities.managers.component_splitter import split_components


class RadarrValidatorManager(BaseManager, ComponentManagerMixin):
    ...
    def __init__(self, logger=None, config=None, global_cache=None, validator=None, registry=None, **kwargs):
        ...
        all_component_classes = {
            "auth":   RadarrValidatorAuthManager,
            "cache":  RadarrValidatorCacheManager,
            "health": RadarrValidatorHealthManager,
            "keys":   RadarrValidatorKeysManager,
        }

        critical_keys = {"auth", "cache", "health", "keys"}
        ...
        for name, cls in critical_components.items():
            try:
                ...
                self.registry.set_flag(f"radarr.validator.{name}_initialized", True)
                self.load_summary[name] = "✅ Loaded"
            except Exception as e:
                self.registry.set_flag(f"radarr.validator.{name}_initialized", False)
        ...
        self.registry.set_flag("radarr.validator_manager_initialized", all_critical_loaded)
