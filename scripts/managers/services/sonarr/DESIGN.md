...
| `GLD-SON-28` | **Audit every `global_cache` accessor name against the real surface** — `load_cache` does not exist on `GlobalCacheManager` or `BaseManager`, yet `storage/library.py` called it four times | `GLD-SON-27` sat undetected because every caller wrapped it in `or {}` or a broad `except`, so an `AttributeError` on EVERY call read as "the library is empty". A name that is wrong at every call site is invisible when failure and emptiness look identical *(**P-D** — failure with no detector)* | S | `GLD-SON-27` |
| `GLD-SON-29` | **Substitute `<instance>` at every `CacheKeyPaths` use site, or make the templates refuse to be f-stringed** | `library.py` appended `.{instance}` instead of replacing the placeholder, naming a literal `<instance>` directory. `key_builder.build_future_episodes_cache_key` does it correctly; nothing enforces the convention, so the two shapes coexist silently | S | `GLD-SON-27` |
| `GLD-SON-30` | **Trace the consumers unblocked by `GLD-SON-27`** — `get_series_by_tvdb`, `get_series_by_title`, `is_series_in_library`, `list_series_by_tag`, `get_all_series_ids`, `get_title_by_series_id`, `has_episode_file` | All were silently seeing an EMPTY library; `has_episode_file` returned False for every episode. Anything that quietly took a "not in library" branch now takes the real one — strictly more correct, but a behaviour change on untraced paths | M | `GLD-SON-27` |
