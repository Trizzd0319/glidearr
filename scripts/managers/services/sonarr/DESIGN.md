...
|---|---|---|---|---|
| `GLD-SON-17` | 🔴 **Size-anomaly remediation NEVER re-grabs — `0 re-grabbed` on every observed run** | Measured 2026-08-10: Sonarr `standard` reported **302 files wildly out of size profile (192 oversized, ~371 GB reclaimable)** and remediated `116 rescanned, 0 re-grabbed, 38 skipped (unmonitored), 2 skipped (guarded)`. Radarr the same shape: `79 rescanned, 0 re-grabbed, 2 skipped`. A rescan corrects the RECORDED size when the cache was merely stale; it does nothing for a file that is genuinely bloated. So ~371 GB sits identified, reported, and untouched, run after run — the operator sees the same Dragon Ball Z lines every pass and nothing changes. **The guard is CORRECT and must not simply be removed** (see `GLD-SON-18`); the defect is that a blocked remediation has no second path and no escalation, so "cannot fix this one" is indistinguishable from "nothing to fix" *(P-D)* | M | — |
| `GLD-SON-18` | 🔴 **Re-grab should SEARCH WITHOUT DELETING** | The armed path is `DELETE episodefile/{fid}` **then** `POST command EpisodeSearch`. Delete-first is what forces the orphan guard to exist: Sonarr will not search for an **unmonitored** episode, so the command is accepted and does nothing, leaving the file deleted and never replaced. Firing `EpisodeSearch` with the file STILL IN PLACE removes that window entirely — Sonarr replaces on import only if it finds something matching the profile, and if it finds nothing the household keeps what it had. This is strictly safer than the current armed path AND it unblocks the 38 skips, because searching an unmonitored episode is merely inert rather than destructive | M | — |
| `GLD-SON-19` | 🟡 **Split `unmonitored` from `unresolved` — they are different problems wearing one message** | `"file backs an unmonitored or unresolved episode"` conflates a POLICY state with a DATA fault. Unmonitored is fixable: the episode exists, and owning a file for it is arguably itself the statement that it is wanted (`PUT episode/{id} monitored:true`, then remediate normally). **Unresolved means the file maps to no known episode at all** — re-grabbing cannot fix it because Sonarr has nothing to search FOR, and it needs reporting, not remediation. Today they share a counter (`skipped_unmonitored`), so the 38 could be 38 policy skips, 38 broken mappings, or any mix, and the log cannot say which *(P-B)* | S | `GLD-SON-18` |
