...
- **Restore.** A coordinator-deleted series whose `watchability_score` later recovers above `owned_restore_score_threshold` (default 20) — e.g. score climbs to 35 > 20 — is re-monitored and `EpisodeSearch`-ed. Unresolved coords are deferred (retried) up to 30 days before being dropped.
